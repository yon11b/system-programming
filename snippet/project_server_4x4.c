...
static void client_process_shm(int cid, int initial_shmid, int domain_shmid, int c2s_shmid) {
    ...
    /* ===================== client → server (Message Queue) ===================== */
    {
        ...
        msg.mtype = cid + 1;   // client id: 1~8

        /* ---------- 첫 번째 256개 ---------- */
        memcpy(msg.data, &domain[0], sizeof(int) * CHUNK_INT);

        if (msgsnd(msqid, &msg, sizeof(msg.data), 0) == -1) {
            perror("msgsnd failed (first 256)");
            exit(1);
        }

        printf("[CLIENT %d] msgsnd 256 ints (domain[0~255])\n", cid);

        /* ---------- 두 번째 256개 ---------- */
        memcpy(msg.data, &domain[CHUNK_INT], sizeof(int) * CHUNK_INT);

        if (msgsnd(msqid, &msg, sizeof(msg.data), 0) == -1) {
            perror("msgsnd failed (second 256)");
            exit(1);
        }

        printf("[CLIENT %d] msgsnd 256 ints (domain[256~511])\n", cid);
    }
